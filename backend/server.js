    ...
    try {
      ...
      if (portError.code === 'EADDRINUSE') {
        ...
        try {
          ...
          console.warn(
            `⚠️  Original port ${port} was in use. Using port ${fallbackPort} instead.`,
          );
...
function isRsvpMutationRequest(req) {
  return (
    req.body &&
    req.body.query &&
    (req.body.query.includes('createRsvp') ||
      req.body.query.includes('updateRsvp'))
  );
}
...
function savePortInfo(port) {
  try {
    ...
    console.error(
      `⚠️  Could not save port information to file: ${error.message}`,
    );
  }
}
