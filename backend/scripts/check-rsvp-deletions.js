...
async function main() {
  try {
    ...
    console.log(
      `Checking for recent activities since ${sevenDaysAgo.toISOString()}`,
    );

    // Check if the collection has any recent changes
    const recentRsvps = await rsvpCollection
      .find({
        $or: [
    ...
    console.log(
      `Found ${recentRsvps.length} RSVPs modified in the last 7 days`,
    );
    ...
    if (recentRsvps.length > 0) {
      fs.writeFileSync(
        './recent-rsvps.json',
        JSON.stringify(recentRsvps, null, 2),
      );
    ...
    const hasProfileCollection = collections.some(
      (col) => col.name === 'system.profile',
    );

    if (hasProfileCollection) {
      console.log(
        'Found system.profile collection, checking for delete operations...',
      );
      const deleteOps = await systemDb
      ...
      console.log(
        `Found ${deleteOps.length} delete operations in profiling data`,
      );

      if (deleteOps.length > 0) {
        fs.writeFileSync(
          './delete-operations.json',
      ...
      console.log(
        'No system.profile collection found (profiling may not be enabled)',
      );
    }
    ...
    const mostRecentRsvps = await rsvpCollection
      .find()
      .sort({ createdAt: -1 })
      .limit(10)
      .toArray();
    ...
    mostRecentRsvps.forEach((rsvp, index) => {
      console.log(
        `${index + 1}. ${rsvp.firstName} ${rsvp.lastName} (${rsvp.email}) - Created: ${rsvp.createdAt}`,
      );
    });
  } catch (error) {
