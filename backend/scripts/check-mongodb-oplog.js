...
async function main() {
  try {
    ...
    try {
      ...
      const hasOplog = await oplogDb.listCollections({ name: 'oplog.rs' }).hasNext();
      
      if (hasOplog) {
        console.log('Found oplog collection, checking for delete operations...');
        ...
        if (deleteOps.length > 0) {
          fs.writeFileSync('./delete-oplog.json', JSON.stringify(deleteOps, null, 2));
        ...
        console.log('No oplog collection found (not available in MongoDB Atlas free tier)');
      }
    } catch (error) {
    ...
    try {
      ...
      if (serverStatus.metrics && serverStatus.metrics.document) {
        console.log(`Documents deleted: ${serverStatus.metrics.document.deleted || 0}`);
      }
