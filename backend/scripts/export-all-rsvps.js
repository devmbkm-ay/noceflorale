...
async function main() {
  try {
    ...
    // Count by attendance
    const attending = allRsvps.filter(rsvp => rsvp.attending === 'attending').length;
    const notAttending = allRsvps.filter(rsvp => rsvp.attending === 'not_attending').length;
    
    // Count by guest type
    const singles = allRsvps.filter(rsvp => rsvp.guestType === 'single').length;
    const couples = allRsvps.filter(rsvp => rsvp.guestType === 'couple').length;
    
    // Count by event participation
    const both = allRsvps.filter(rsvp => rsvp.eventParticipation === 'both').length;
    const blessingOnly = allRsvps.filter(rsvp => rsvp.eventParticipation === 'blessing_only').length;
    const eveningOnly = allRsvps.filter(rsvp => rsvp.eventParticipation === 'evening_only').length;
    ...
    console.log(`Participation - Both: ${both}, Blessing Only: ${blessingOnly}, Evening Only: ${eveningOnly}`);
    ...
    console.log(`Total Guest Count (including partners and children): ${totalGuests}`);
    
    // Create a CSV export for easier viewing in spreadsheets
    const csvRows = [];
    csvRows.push('First Name,Last Name,Email,Telephone,Attending,Guest Type,Event Participation,Partner First Name,Partner Last Name,Partner Email,Partner Telephone,Partner Event Participation,Created At');
